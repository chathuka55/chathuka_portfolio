...
class CarLights {
  ...
  init() {
    ...
    if (Array.isArray(this.colors)) {
      ...
      colorArray = [new THREE.Color(this.colors as THREE.ColorRepresentation)];
    }
...
function mergeHyperspeedOptions(effectOptions: Partial<HyperspeedOptions>): HyperspeedOptions {
  const merged: HyperspeedOptions = {
    ...defaultOptions,
    ...effectOptions,
    colors: { ...defaultOptions.colors, ...effectOptions.colors },
  };
  if (typeof merged.distortion === 'string') {
    const key = merged.distortion;
    merged.distortion = distortions[key] ?? distortions.turbulentDistortion;
  }
  return merged;
}
...
const Hyperspeed: FC<HyperspeedProps> = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }) => {
  ...
  useEffect(() => {
    ...
    const startId = requestAnimationFrame(() => {
      if (cancelled || !hyperspeed.current) return;
      const container = hyperspeed.current;
      const options = mergeHyperspeedOptions(effectOptions);
      try {
        ...
        ro = new ResizeObserver(() => myApp?.onWindowResize());
        ro.observe(container);
      } catch (e) {
        console.error('[Hyperspeed] WebGL init failed:', e);
      }
    });
  ...
  return (
    <div
      className="pointer-events-none absolute inset-0 h-full min-h-full w-full [&_canvas]:block [&_canvas]:h-full [&_canvas]:w-full [&_canvas]:max-h-none"
      ref={hyperspeed}
    />
  );
};
