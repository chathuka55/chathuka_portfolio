...
export default function LightPillar({
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error('[LightPillar] WebGL renderer failed:', error);
    ...
    /* Scene rebuild only when quality tier changes; colors/size/etc. update via uniforms below */
  ...
  if (!webGLSupported) {
    return (
      <div
        className={`absolute left-0 top-0 flex h-full w-full items-center justify-center bg-[#0a0a0a] text-white/40 text-sm ${className}`}
        style={{ mixBlendMode }}
      >
        WebGL not available
      </div>
